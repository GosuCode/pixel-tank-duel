...
const {
  ...
  BULLET_R, BULLET_SPEED, BULLET_MAX_DISTANCE, FIRE_COOLDOWN,
  ...
  TAUNT_COOLDOWN, TAUNT_DURATION, EXPLOSION_DURATION, HIT_EVENT_DURATION,
...
function buildPowerupPlan(startAt, spots) {
  // Fixed per-map spots and a fixed type order on a fixed clock, so the plan is
  // identical every round. One entry per type so every powerup appears.
  const plan = [];
  for (let i = 0; i < POWERUP_TYPES.length; i++) {
    const spot = spots[i % spots.length];
    plan.push({ at: startAt + POWERUP_FIRST_AT + i * POWERUP_INTERVAL, x: spot.x, y: spot.y, type: POWERUP_TYPES[i] });
  }
  return plan;
}
...
function createRoom(code, visibility = 'open') {
  ...
  function startRound() {
    ...
    round.powerupPlan = buildPowerupPlan(round.countdownEndAt, POWERUP_SPOTS);
