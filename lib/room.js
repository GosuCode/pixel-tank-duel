...
function createRoom(code, visibility = 'open', opts = {}) {
  const sockets = new Set(); // ws connections currently in this room
  const createdAt = Date.now();
  const sandbox = !!opts.sandbox; // dev playground: bots + debug actions
  ...
  if (sandbox) {
    // Playground: never "win" the match, and stock the arena with live targets.
    match.targetWins = 99;
    addBot();
    addBot();
  }
  ...
  // Shared player factory so humans and sandbox bots start from one shape.
  function makePlayer(index, bot) {
  ...
  function addBot() {
    if (Object.keys(players).length >= MAX_PLAYERS) return null;
    const index = Object.keys(players).length;
    const id = `bot${++botSeq}`;
    players[id] = makePlayer(index, true);
    maybeStartRound();
    return id;
  }

  function removeBot() {
    const ids = Object.keys(players).filter((id) => players[id].bot);
    if (!ids.length) return false;
    delete players[ids[ids.length - 1]];
    return true;
  }

  // Sandbox AI: seek the nearest enemy, strafe, and fire when roughly lined up.
  function updateBots(now) {
    for (const id in players) {
      ...
      if (!target) { p.fire = false; continue; }
      const tx = target.x - p.x;
      const ty = target.y - p.y;
      const dist = Math.sqrt(best) || 1;
      const aim = Math.atan2(ty, tx);
      // imperfect aim so the player can actually test without instant death
      p.aim = aim + (Math.random() - 0.5) * 0.28;
      p.aimControlled = true;
      const sign = dist > 240 ? 1 : dist < 130 ? -1 : 0;
      const strafe = Math.sin(now / 900 + p.aiDir) * 0.9;
      let mvx = (tx / dist) * sign + (-ty / dist) * strafe;
      let mvy = (ty / dist) * sign + (tx / dist) * strafe;
      const ml = Math.hypot(mvx, mvy) || 1;
      p.dx = mvx / ml;
      p.dy = mvy / ml;
      // nudge away from walls/obstacles
      const nx = p.x + p.dx * (TANK_R + 8);
      const ny = p.y + p.dy * (TANK_R + 8);
      const off = nx < TANK_R || nx > ARENA_W - TANK_R || ny < TANK_R || ny > ARENA_H - TANK_R
        || OBSTACLES.some((o) => circleRectCollide(nx, ny, TANK_R, o));
      if (off) {
        p.aiDir = Math.atan2(CY - p.y, CX - p.x) + (Math.random() - 0.5);
        p.dx = Math.cos(p.aiDir);
        p.dy = Math.sin(p.aiDir);
        p.aim = aim;
      }
      // fire on a short reaction cadence so the player isn't insta-melted
      p.fire = dist < 380 && Math.random() < 0.045;
    }
  }
  ...
  function endRound(winnerId, winnerText) {
    ...
    for (const id in players) {
      const p = players[id];
      if (p.bot) continue; // sandbox bots have no bank
  ...
  function maybeStartRound() {
    const ids = Object.keys(players);
    const humans = ids.filter((id) => !players[id].bot).length;
    // Bots alone never start a round; a human must be present (sandbox play).
    if (round.state === 'waiting' && ids.length >= 2 && humans >= 1) {
      startRound();
    }
  }
  ...
  function bumpHaunt(p) {
    ...
    p.streak = 0;
    p.hauntLevel = Math.min(2, Math.max(1, Math.floor(p.lossStreak / 2) + 1));
  }
  ...
  function join(ws) {
    ...
    const index = Object.keys(players).length;
    players[id] = makePlayer(index, false);
    players[id].ws = ws;
    ...
    ws.on('message', (message) => {
      ...
      if (data.type === 'input') {
        ...
        if (tok && !p.token) {
          p.token = tok;
          const auth = resolveAuth(tok);
          if (auth.status === 'pending') {
            // a linked device waits for the owner to approve it before any bank access
            p.pending = true;
            try {
              ws.send(JSON.stringify({ type: 'pendingApproval', requestId: auth.requestId, label: auth.label }));
            } catch (e) {}
          } else {
            if (auth.accountId) p.accountId = auth.accountId;
            touchDevice(tok);
        ...
        bankOf(p); // ensure the account exists before any legacy callsign-bank adoption
      ...
      } else if (data.type === 'dbg' && sandbox) {
        // Playground controls: only honoured inside a sandbox room.
        const a = players[id];
        if (!a) return;
        const now = Date.now();
        if (data.action === 'grant' && POWERUP_TYPES.includes(data.powerup)) {
          a.buff = { type: data.powerup, expiresAt: now + POWERUP_DURATION };
        } else if (data.action === 'spawnpowerup' && POWERUP_TYPES.includes(data.powerup)) {
          powerup = { x: a.x + Math.cos(a.aim) * 46, y: a.y + Math.sin(a.aim) * 46, type: data.powerup };
        } else if (data.action === 'clearpower') {
          a.buff = null;
        } else if (data.action === 'heal') {
          a.hp = MAX_HP;
          a.alive = true;
        } else if (data.action === 'god') {
          a.god = !a.god;
          ...
          startRound();
        } else if (data.action === 'bolts') {
          a.bolts = credit(a, 100);
          sendBank(a, { type: 'debug' });
        } else if (data.action === 'haunt') {
          // set the haunt level directly so effects can be previewed on demand
          const lv = Math.max(0, Math.min(2, Number(data.level) || 0));
          a.hauntLevel = lv;
          a.lossStreak = lv <= 1 ? lv : (lv - 1) * 2;
        } else if (data.action === 'immunity') {
          // grant one Haunt Immunity token so the blocked path can be tested
          const b = bankOf(a);
          if (b) {
            b.bought.immunity = (b.bought.immunity || 0) + 1;
            a.bought = b.bought;
            saveBank();
            sendBank(a, { type: 'debug' });
          }
  ...
  function tick() {
    ...
    for (const id in players) {
      ...
      const cooldown = isLaser ? LASER_COOLDOWN : buffType === 'rapid' ? RAPID_COOLDOWN : FIRE_COOLDOWN;
      // Lasers charge: hold fire for LASER_CHARGE before the shot releases.
      let readyToFire = false;
      if (isLaser) {
        if (!frozen && p.fire && inPlay) {
          if (!p.chargeStart) p.chargeStart = now;
          readyToFire = now - p.chargeStart >= LASER_CHARGE && now - p.lastShot > cooldown;
        } else {
          p.chargeStart = 0;
        }
      } else {
        p.chargeStart = 0;
        readyToFire = !frozen && p.fire && inPlay && now - p.lastShot > cooldown;
      }
      if (readyToFire) {
        p.lastShot = now;
        if (isLaser) p.chargeStart = now; // begin recharging for the next shot
      ...
      bullets = bullets.filter((b) => {
        ...
        for (const id in players) {
          ...
          if (dx * dx + dy * dy < (TANK_R + BULLET_R) * (TANK_R + BULLET_R)) {
            ...
            let kind = 'damage';
            let hitDmg = 0;
            if (now < target.spawnProtectedUntil || target.god) {
              // spawn protection / playground god mode: shot consumed, no damage
              kind = 'absorbed';
            } else if (target.buff && target.buff.type === 'shield') {
              target.buff = null; // shield absorbs the hit and shatters
              kind = 'absorbed';
            } else {
              const dmg = b.laser ? 2 : 1; // a laser burns through two HP
              target.hp -= dmg;
    ...
    for (const id in players) {
      ...
      playersOut[id] = {
        ...
        charge: p.buff && p.buff.type === 'laser' && p.chargeStart
          ? Math.min(1, (now - p.chargeStart) / LASER_CHARGE)
          : 0,
    ...
    const state = JSON.stringify({
      ...
      hitEvents: hitEvents.map((e) => ({ id: e.id, x: e.x, y: e.y, by: e.by, target: e.target, kind: e.kind, dmg: e.dmg })),
